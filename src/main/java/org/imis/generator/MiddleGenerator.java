...
public class MiddleGenerator extends Constant {
    String ontology;
    ...
    public ArrayList<UnderCourseInstance> c_Courses = new ArrayList<>();
    ...
    public IdGenerator idGen = new IdGenerator();
    ...
    public void _generateDept(int univIndex, int index) {
        ...
        String dirPath = path + _getName(CS_C_UNIV, univIndex) + INDEX_DELIMITER + index;
        new File(dirPath).mkdirs();
        // String fileName = System.getProperty("user.dir") + "\\Csv\\" + _getName(CS_C_UNIV, univIndex) + INDEX_DELIMITER + index + _getFileSuffix();
        writer_.startFile(dirPath);
   ...
   /* _generateConferencePublications();
     ...
     */
    private void _generateAFaculty_a(int type, int index, String id,ProfessorInstance p) {
        int indexInFaculty;
        int courseNum;
        int courseIndex;
        boolean dup;
        ...
        writer_.addProperty(CS_P_NAME, name, false);
        if(globalVersionTrigger){
            writer_log.addPropertyInstance(id, ontology+"#name", name, false );
        }
        p.setName(name);
        ...
        for (int i = 0; i < courseNum; i++) {
            ...
            UnderCourseInstance ci = new UnderCourseInstance();
            ci.setId(idGen.getNextId());

            courseIndex = _AssignCourse(indexInFaculty);
            ci.setName(_getRelativeName(CS_C_COURSE,courseIndex));
            cos = _getId(CS_C_COURSE, courseIndex);
            courses.add(cos);
            writer_.addProperty(CS_P_TEACHEROF, cos, true);
            if(globalVersionTrigger){
                writer_log.addPropertyInstance(id, ontology+"#teacherOf", cos, true );
            }
            c_Courses.add(ci);
            p.addCourse(ci.getId());
        ...
        for (int i = 0; i < courseNum; i++) {
            ...
            ci.setName(_getRelativeName(CS_C_GRADCOURSE,courseIndex));

            cos = _getId(CS_C_GRADCOURSE, courseIndex);
            courses.add(cos);
            writer_.addProperty(CS_P_TEACHEROF, cos, true);
            if(globalVersionTrigger){
                writer_log.addPropertyInstance(id, ontology+"#teacherOf", cos, true );
        ...
        for (int i = 0; i < courseNum; i++) {
            ...
            wi.setId(idGen.getNextId());



            courseIndex = _AssignWebCourse(indexInFaculty);

            wi.setName(_getRelativeName(CS_C_WEBCOURSE,courseIndex));
            cos = _getId(CS_C_WEBCOURSE, courseIndex);
            courses.add(cos);
            writer_.addProperty(CS_P_TEACHEROF, cos, true);
            if(globalVersionTrigger){
                writer_log.addPropertyInstance(id, ontology+"#teacherOf", cos, true );
        ...
        //person properties
        String undergraduateDegreeFrom,mastersDegreeFrom,doctoralDegreeFrom;
        ...
        String worksfor = _getId(CS_C_DEPT, instances_[CS_C_DEPT].count - 1);
        writer_.addProperty(CS_P_WORKSFOR, worksfor, true);
        if(globalVersionTrigger){
            writer_log.addPropertyInstance(id, ontology+"#worksFor",  worksfor, true );
        }
        String email = _getEmail(type,index);
        writer_.addProperty(CS_P_EMAIL, email, false);
        if(globalVersionTrigger){
            writer_log.addPropertyInstance(id, ontology+"#email",  email, false );
        }
        String telePhone = "xxx-xxx-xxxx";
        ...
        p.setUndergraduateDegreeFrom(undergraduateDegreeFrom);
        p.setMastersDegreeFrom(mastersDegreeFrom);
        p.setDoctoralDegreeFrom(doctoralDegreeFrom);
        p.setWorksFor(worksfor);
    ...
    private void _generateAProf(int index) {
        ...
        _generateAProf_a(CS_C_PROF, index, _getId(CS_C_PROF, index),p);
     ...
     */
    private void _generateAProf_a(int type, int index, String id,ProfessorInstance p) {
        _generateAFaculty_a(type, index, id,p);
    ...
    private void _generateAFullProf(int index) {
        ...
        _generateAProf_a(CS_C_FULLPROF, index, id,p);
        if (index == chair_) {
            p.setChair(true);
        ...
        _assignFacultyPublications(id, FULLPROF_PUB_MIN, FULLPROF_PUB_MAX,p);
        c_Professors.add(p);
    }
    ...
    private void _generateAVisitingProf(int index) {
        ...
        _generateAProf_a(CS_C_VISITINGPROF, index, id,p);
    ...
    private void _generateAnAssociateProfessor(int index) {
        ...
        _generateAProf_a(CS_C_ASSOPROF, index, id,p);
        writer_.endSection(CS_C_ASSOPROF);
        _assignFacultyPublications(id, ASSOPROF_PUB_MIN, ASSOPROF_PUB_MAX,p);
        c_Professors.add(p);
    }
    ...
    private void _generateAnAssistantProfessor(int index) {
        ...
        _generateAProf_a(CS_C_ASSTPROF, index, id,p);
        writer_.endSection(CS_C_ASSTPROF);
        _assignFacultyPublications(id, ASSTPROF_PUB_MIN, ASSTPROF_PUB_MAX,p);
        c_Professors.add(p);
    }
    ...
    private void _generateALecturer(int index) {
        ...
        _generateAFaculty_a(CS_C_LECTURER, index, id,p);
        writer_.endSection(CS_C_LECTURER);
        _assignFacultyPublications(id, LEC_PUB_MIN, LEC_PUB_MAX,p);
        c_Professors.add(p);
    }
     ...
     */
    private void _assignFacultyPublications(String author, int min, int max,ProfessorInstance p) {
        int num;
